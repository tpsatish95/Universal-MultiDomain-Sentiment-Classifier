...
class SentiHandle(object):
	...
	def getSentimentScore(self,mainText,textType = "microblogs", title="",middleParas = [],lastPara = "",topDomain="",subDomain=""):
		...
		blogs_news - mainText< or first paragraph>, title, textType="blogs_news",lastPara = "" <optional last paragraph>,middleParas = [] <optional list of middle paragraphs>
		'''
		...
		if textType == "general":
			...
			score = self.blogs_news.getSentimentScore(title,mainText,lastPara,middleParas)
			scaledScore = self.scalerLargeText.scaleScore(score)
